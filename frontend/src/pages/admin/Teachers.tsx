...
export default function Teachers() {
  ...
  const [loginTarget, setLoginTarget] = useState<Teacher | null>(null);
  ...
  return (
    <div>
                ...
                <tr key={t.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50">
                  <td className="px-4 py-2.5 text-slate-900 font-medium">
                    <div className="flex items-center gap-2">
                      {t.name}
                      {t.login_username && (
                        <span className={`inline-flex items-center gap-1 px-2 py-0.5 text-xs rounded-full ${t.login_active ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}>
                  ...
                  <td className="px-4 py-2.5 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <button onClick={() => setLoginTarget(t)} className="p-1.5 text-slate-400 hover:text-[#14532D] rounded" title="Manage Login">
      ...
      <TeacherLoginModal cls={loginTarget} onClose={() => { setLoginTarget(null); fetchTeachers(); }} />
    </div>
  );
}
...
function TeacherLoginModal({ cls, onClose }: { cls: Teacher | null; onClose: () => void }) {
  ...
  useEffect(() => {
    if (cls) {
      setUsername(cls.login_username || cls.name.toLowerCase().replace(/\s+/g, '.'));
      setPassword('');
    }
  }, [cls]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await api.post(`/teachers/${cls.id}/create-login`, { username, password });
      addToast('Teacher login created/updated successfully', 'success');
      onClose();
    } catch (err: any) {
  ...
  return (
    <Modal isOpen={true} onClose={onClose} title={`Login for Teacher ${cls.name}`}>
      <form onSubmit={handleSubmit} className="space-y-4">
        <p className="text-sm text-slate-600">
          {cls.login_username
            ? `This teacher already has a login account (${cls.login_username}). You can update the credentials below.`
            : 'Create a login account for this teacher. They can use these credentials to access the teacher dashboard.'}
        </p>
        <div>
